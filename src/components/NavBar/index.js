...
class NavBar extends Component {
   ...
   */
  openSearch() {
    this.setState({ toSearchPage: false, search: "" }, () =>
  ...
  render() {
    ...
    return (
      <Segment basic color="purple" inverted size="small" className="nav-bar">
        ...
        {this.state.toSearchPage !== false && searchVisible ? (
          <Redirect to={`/search/${this.state.toSearchPage}`} />
        ) : null}
      </Segment>
    );
  }
...
export default connect(mapStateToProps, { openMenu, openSearch, closeSearch })(
  NavBar
);
